...
function lucasNumber(n) {
  ...
  let x =  lucasNumber(n - 2) + lucasNumber(n - 1);
  return x;
}
...
function reverseString(str) {
  ...
  let lastChar = str[str.length - 1];
	let trimString = str.slice(0, -1);
	return lastChar + reverseString(trimString);
}
...
function fileFinder(directories, targetFile) {
  let fileNames = Object.keys(directories);
  let foundFile = fileNames.some(fileName => {
  ...
  let values = Object.values(directories);
	
	foundFile = values.some((object) => {
    if (object instanceof Object) {
      if ( fileFinder(object, targetFile) ) {
        return true;
      }
		} 
	});
...
function pathFinder(directories, targetFile) {
  ...
  fileNames.forEach(fileName => {
    ...
    if ( directories[fileName] ) {
      ...
      let something = pathFinder(directories[fileName], targetFile);
      if ( something !== null ) {
        path = fileName + something;
      }
    }
  });
